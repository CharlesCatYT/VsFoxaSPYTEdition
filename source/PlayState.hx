...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		PlayStateChangeables.noteSplashes = FlxG.save.data.notesplash;
		...
		switch (songLowercase)
		{
			...
			case 'senpai' | 'roses' | 'thorns' | 'bubbles' | 'burning-flames' | 'execution':
		...
		if (SONG.stage == null)
		{
			...
			{
				/*case 0:
					if (songLowercase == 'execution')
					{
						stageCheck = 'ballisticAlley'; // this shit aint working :[
					}
					else if (songLowercase == 'firestorm')
					{
						stageCheck = 'ballisticAlley';
					}
					else
					{
						stageCheck = 'alley';
				...
				case 'alley':
					{
						defaultCamZoom = 0.9;
						curStage = 'alley';
						wBg = new FlxSprite(-500, -300).loadGraphic(Paths.image('whittyBack', 'foxa'));

						wBg.antialiasing = true;
						wBg.scrollFactor.set(0.9, 0.9);
						wBg.active = false;

						wstageFront = new FlxSprite(-650, 600).loadGraphic(Paths.image('whittyFront', 'foxa'));
				...
				case 'ballisticAlley':
					{
						...
						trace('pogging');
						wBg.antialiasing = true;
						var bgTex = Paths.getSparrowAtlas('BallisticBackground', 'foxa');
						...
						wBg.alpha = 0;
						nwBg.alpha = 1;
						funneEffect = new FlxSprite(-600, -200).loadGraphic(Paths.image('thefunnyeffect', 'foxa'));
		...
		switch (SONG.player2)
		{
			...
			case 'foxa-mad':
				if (FlxG.save.data.distractions)
				{
					...
					if (!PlayStateChangeables.Optimize)
					{
						var afterImage = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
						// afterImage.changeValuesEnabled(false, false, false, false);
						// afterImage.changeGraphic()
						add(afterImage);
					}
					// afterImage.scrollFactor.set(1.1, 1.1);
				}
				camPos.x += 400;
				camPos.y += 200;
			case 'pico':
		...
		if (FlxG.save.data.colour)
		{
			switch (SONG.player2)
			{
				...
				case 'foxa' | 'foxa-angy':
					healthBar.createFilledBar(0xFFFFD1FF, 0xFF0097C4);
				case 'foxa-mad':
					healthBar.createFilledBar(0xFF000000, 0xFF0097C4);
			}
		}
		else
		...
		{
			case 'firestorm':
				credits = 'Original Song made by Garageband Cover Guy 96!';
			default:
				credits = '';
		}
		...
		var kadeEngineWatermark = new FlxText(4, textYPos, 0,
			SONG.song
			+ " "
			+ (Main.watermarks ? " | KE FOXA/" + MainMenuState.kadeEngineVer : ""), 16);
		kadeEngineWatermark.setFormat(Paths.font("funkyfont.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (creditsText)
		{
			...
			creditsWatermark.setFormat(Paths.font("funkyfont.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("funkyfont.ttf"), 16, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		replayTxt = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), 0, "REPLAY",
			20);
		replayTxt.setFormat(Paths.font("funkyfont.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botPlayState = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), 0,
			"BOTPLAY", 20);
		botPlayState.setFormat(Paths.font("funkyfont.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				...
				/*var blackScreen:FlxSprite = new FlxSprite(0, 0).makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.BLACK);
					...
					{
						remove(blackScreen);
						//FlxG.sound.play(Paths.sound('arson','foxa')); //file doesn't exist yet
						camFollow.y = 590;
						camFollow.x -= 200;
						FlxG.camera.focusOn(camFollow.getPosition());
						...
						{
						dad.playAnim('singUP', true); //temporary animation for now
						});
				...
				case 'execution':
					...
					else
					{
						...
						{
							remove(blackScreen);
							FlxG.sound.play(Paths.sound('boom', 'foxa')); // placeholder vine boom
							camFollow.y = 590;
							camFollow.x -= 200;
							FlxG.camera.focusOn(camFollow.getPosition());
							// FlxG.camera.zoom = 1.5;
		...
		{
			if (!PlayStateChangeables.Optimize) // no more crashing in optimization mode
			{
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		senpaiEvil.frames = Paths.getSparrowAtlas('senpaiCrazy', 'shared');
	...
	function ExecuteScene()
	{
		...
		FlxG.sound.play(Paths.sound('crack', 'foxa'));
	}
	...
	public function generateSong(dataPath:String):Void
	{
		...
		if (FlxG.save.data.songPosition)
		{
			...
			if (PlayStateChangeables.useDownscroll)
				songName.y -= 3;
			songName.setFormat(Paths.font("funkyfont.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			songName.scrollFactor.set();
			add(songName);
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (FlxG.save.data.midscroll) // this should entirely remove the opponent notes with both optimization and middlescroll on
				{
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (FlxG.save.data.midscroll)
			{
				...
				{
					case 0:
						babyArrow.x -= 550; // if you cant remove them, put them offscreen
					case 1:
						babyArrow.x -= 275;
				}
			}
	...
	override public function update(elapsed:Float)
	{
		...
		{
			botplaySine += 180 * elapsed;
			replayTxt.alpha = 1 - Math.sin((Math.PI * botplaySine) / 180);
		}
		...
		if (curStage == 'ballisticAlley')
		{
			...
			if (healthBar.percent < 20)
			{
				FlxTween.tween(funneEffect, {alpha: 1}, 1, {ease: FlxEase.expoInOut});
				add(funneEffect);
			}
		}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
					...
					&& daNote.mustPress)
				{
					...
					else if (daNote.nType == 'warning')
					{
						health -= 15.05;
					}
					else if (daNote.nType == 'bad')
					{
						// do nothing
					}
					else
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					switch (curSong)
					{#if !html5 case "Bubbles":
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/burningflamesCutscene.webm", new PlayState())); #end
						default:
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		if (daRating != 'shit' || daRating != 'bad')
		{
			...
			if (PlayStateChangeables.noteSplashes)
			{
				{
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		if (PlayStateChangeables.botPlay)
			notes.forEachAlive(function(daNote:Note)
			{
				...
				{
					if (daNote.nType != 'bad') // makes bot miss on bad notes
					{
	...
	override function beatHit()
	{
		...
		if (!PlayStateChangeables.Optimize)
		{
			if (curSong == "Burning-Flames")
			{
				switch (curBeat)
				{
					case 84:
						remove(dad);
						dad = new Character(100, 100, 'foxa-angy-alt');
						trace("Character Change!");
						add(dad);

					case 96:
						remove(dad);
						dad = new Character(100, 100, 'foxa-angy');
						trace("Character Change!");
						add(dad);
				}
			}
		...
		if (curSong.toLowerCase() == 'burning-flames' && curBeat == 286 && isStoryMode)
		{
			ExecuteScene();
		}
		else if (curSong.toLowerCase() == 'burning-flames' && curBeat == 286 && !isStoryMode)
		{
			endSong();
		}
